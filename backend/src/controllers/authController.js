...
export const updateProfile = async (req,res) => {
    ...
    if(!userId) return res.status(400).json({success:false,message:'Not Authorized , Login Again'})

    if(!profilePic) return res.status(400).json({success:false,message:'Add Profile pic'})
    
    try{

      const uploadResponse = await cloudinary.uploader.upload(profilePic);

     const user = await userModel.findByIdAndUpdate(userId, {profilePic: uploadResponse.secure_url}, {new:true});

    await user.save();

    return res.status(200).json({success:true,message:'Profile pic updated successfully'})
